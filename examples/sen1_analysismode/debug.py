...
def fit_orbit(
    ...
    polyfit_sat_pos = sat_pos.polyfit(dim=time_dim, deg=deg)
    coeff = polyfit_sat_pos.polyfit_coefficients
...
def zero_doppler_plane_distance_velocity(
    ...
    polyfit_orbit: xr.DataArray,
    orbit_time: xr.DataArray,
    dim: str = "axis",
) -> tuple[xr.DataArray, tuple[xr.DataArray, xr.DataArray]]:
    sat_pos = xr.polyval(orbit_time, polyfit_orbit)
    dem_dist = dem_ecef - sat_pos
    sat_vel = xr.polyval(orbit_time, polyder(polyfit_orbit))
...
def zero_doppler_plane_distance_velocity_prime(
    ...
    acceleration = xr.polyval(orbit_time, polyder(polyder(polyfit_orbit)))
...
def backward_geocode_simple(
    ...
    zero_doppler = functools.partial(
        zero_doppler_plane_distance_velocity, dem_ecef, polyfit_orbit
    )

    orbit_time_guess = xr.full_like(dem_ecef.x, 0, dtype="float64")
    ...
    if method == "secant":
        ...
        zero_doppler_prime = functools.partial(
            zero_doppler_plane_distance_velocity_prime, polyfit_orbit
        )
    ...
    acquisition = xr.Dataset(
        data_vars={
            "azimuth_time": orbit_time_to_azimuth_time(
                orbit_time, polyfit_orbit.attrs["epoch"]
            ),
...
def simulate_acquisition(
    ...
    acquisition = backward_geocode(dem_ecef, polyfit_orbit, azimuth_time)
...
def map_simulate_acquisition(
    ...
    acquisition = xr.map_blocks(
        ...
        kwargs={
            "orbit_interpolator": polyfit_orbit,
...
def azimuth_slant_range_grid(
    ...
    slant_range_spacing_m = (
        attrs["stac_discovery"]["range_pixel_spacing"] * grouping_area_factor[1]
    )
    slant_range_time_interval_s = (
...
def do_terrain_correction(
    ...
    polyfit_orbit = fit_orbit(orbit["position"])
    ...
    if correct_radiometry is not None:
        ...
        simulated_beta_nought.lon.attrs.update(dem_ecef.lon.attrs)
        simulated_beta_nought.lat.attrs.update(dem_ecef.lat.attrs)
    ...
    ground_range = self.slant_range_time_to_ground_range(azimuth_time, slant_range_time)
...
if __name__ == "__main__":
    ...
    # Apply GTC
    ds_corrected = do_terrain_correction(dt, dem)
    ds_corrected.plot(vmin=0, vmax=1)
    plt.show()
